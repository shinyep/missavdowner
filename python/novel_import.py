...
def import_video_to_novel(
    novel_project_path: str,
    output_path: str,
    title: str,
    cover_url: str = '',
    source: str = 'missav',
    ...
    lines = [
        ...
        f"title = r'{ti}'",
        f"output_path = r'{op}'",
        f"gallery_id = {gallery_id if gallery_id is not None else 'None'}",
        "",
...
def import_images_to_novel(novel_project_path: str, title: str, image_paths: list[str]) -> dict:
    ...
    django_script = '\n'.join([
        ...
        "    base = (name or '').lower()",
        "    return re.sub(r'^\\d+_\\d{3}', '', base)",
        f'title = {repr(safe_title)}',
        f'image_paths = {repr(existing_paths)}',
        'gallery, created = Gallery.objects.get_or_create(title=title)',
        "existing_source_names = set()",
        "for caption in GalleryImage.objects.filter(gallery=gallery).values_list('caption', flat=True):",
        "    normalized = normalize_source_name(caption or '')",
        "    existing_source_names.add(normalized.lower())",
        ...
        '    if not path.exists():',
        '        continue',
        "    source_name = normalize_source_name(path.name)",
